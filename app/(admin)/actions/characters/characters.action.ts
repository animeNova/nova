...
import {  charachter as Character, show } from "@/drizzle/db/schema";
import { z } from "zod";
import { characterSchema } from "../../types/zod.types";
import { revalidatePath } from "next/cache";

export const getCharacters = async (props : QueryPorps) => {
    const {limit = 12,orderBy = 'desc',page = 1} = props;

    const offset = (page - 1) * limit;
    const totalCharacterResult  = await db.select({count: count() }).from(Character);
    ...
    }).from(Character).leftJoin(show, eq(show.id,Character.showId)).orderBy(orderBy == 'asc' ? asc(Character.createdAt) : desc(Character.createdAt)).offset(offset).limit(limit)
...
export const deleteCharacter = async (id : string) => {
    const exist = await db.query.charachter.findFirst({
        where : (_Character , {eq}) => eq(_Character.id,id),
    });
    if(!exist){
        return {error : "Character was Not Found!"}
    }
    await db.delete(Character).where(eq(Character.id,exist.id));
    revalidatePath('/admin/Characters')
    return {
